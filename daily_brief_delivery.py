...
class AuthorizationContext:
    scopes:frozenset[str]=frozenset(); self_access:bool=True
@dataclass
class RetryPolicy:
    max_attempts:int=3; retryable:frozenset[str]=frozenset({'TEMPORARY_PROVIDER_ERROR','TIMEOUT','RATE_LIMIT'})
@dataclass
class DailyBriefDeliveryPolicy:
    enabled:bool=True; only_if_relevant:bool=True; send_all_clear:bool=False; preferred_time:str='08:00'; timezone:str='Europe/Berlin'
    allowed_channels:tuple[Channel,...]=(Channel.WHATSAPP,Channel.APP_PUSH,Channel.WEB); fallback_channels:tuple[Channel,...]=(Channel.APP_PUSH,)
    quiet_hours:tuple[str,str]=('22:00','08:00'); weekend_policy:str='USER_PREFERENCE'; max_deliveries_per_day:int=2
    retry_policy:RetryPolicy=field(default_factory=RetryPolicy); voice_allowed:bool=False; email_allowed:bool=False; family_scope_required:bool=True
    sensitivity_ceiling:Sensitivity=Sensitivity.SENSITIVE
@dataclass(frozen=True)
class DailyBriefDeliveryRequest:
    delivery_request_id:str; brief_id:str; person_id:str; account_id:str; requested_mode:str; preferred_channel:Channel; fallback_channels:tuple[Channel,...]
    timezone:str; scheduled_for:datetime; delivery_policy:DailyBriefDeliveryPolicy; authorization_context:AuthorizationContext; idempotency_key:str; created_at:datetime
@dataclass(frozen=True)
class ChannelCapabilities:
    supports_text:bool=False; supports_audio:bool=False; supports_html:bool=False; supports_push:bool=False; supports_receipts:bool=False; supports_read_receipts:bool=False; supports_links:bool=False; supports_buttons:bool=False
CAPABILITIES={Channel.WHATSAPP:ChannelCapabilities(True,True,False,False,True,True,True,True),Channel.APP_PUSH:ChannelCapabilities(True,False,False,True,True,False,True,False),Channel.EMAIL:ChannelCapabilities(True,False,True,False,True,False,True,False),Channel.VOICE:ChannelCapabilities(False,True,False,False,True,False,False,False),Channel.WEB:ChannelCapabilities(True,True,True,False,False,False,True,True)}
FEATURE_FLAGS={'daily_brief.on_demand':False,'daily_brief.whatsapp_delivery':False,'daily_brief.push_delivery':False,'daily_brief.email_delivery':False,'daily_brief.voice_delivery':False,'daily_brief.scheduled_delivery':False}
@dataclass
...
class DeliveryRecord:
    request:DailyBriefDeliveryRequest; state:DeliveryState=DeliveryState.PLANNED; worker_id:str|None=None; lease_until:datetime|None=None; attempt:int=0; receipt:Receipt=Receipt.UNKNOWN; provider_calls:int=0; audit:list=field(default_factory=list)
...
class Renderer:
    ...
    def render(channel,brief,sensitivity=Sensitivity.NORMAL):
        headline=brief.get('headline',''); points=list(brief.get('top_items',[]))[:3]
...
class DeliveryPlanner:
    @staticmethod
    def deterministic_id(person_id,date,mode,data_snapshot,channel): return hashlib.sha256('|'.join(map(str,(person_id,date,mode,data_snapshot,channel.value))).encode()).hexdigest()
    @staticmethod
    def in_quiet_hours(now,quiet):
        start,end=(time.fromisoformat(x) for x in quiet); t=now.time(); return (t>=start or t<end) if start>end else start<=t<end
    @staticmethod
    ...
    def fallback(req,failed_channel):
        ...
        for c in req.fallback_channels:
