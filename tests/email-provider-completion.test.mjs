...
assert.ok(ui.includes('src="/assets/webde-pop-aktivieren.mp4"'));
assert.ok(ui.includes('playsinline muted controls hidden'));
assert.ok(ui.includes('function toggleWebdeGuideVideo()'));
const videoToggleStart = ui.indexOf('function toggleWebdeGuideVideo()');
const webdeAudioStopStart = ui.indexOf('function stopWebdeGuideAudio()');
assert.ok(videoToggleStart >= 0 && webdeAudioStopStart > videoToggleStart);
assert.equal(ui.slice(videoToggleStart, webdeAudioStopStart).includes('stopWebdeGuideAudio()'), false);
