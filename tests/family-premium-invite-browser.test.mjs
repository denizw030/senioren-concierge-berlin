...
const familyJs="/assets/family-owner-sponsored-access.js";
...
(() => {
  ...
  window.fetch=async(input,init={})=>{
    ...
    if(url.pathname==="/api/runtime-config"){
      return new Response(JSON.stringify({ok:true,family_contract:"family-owner-sponsored-access-v1",platform_contract_sha:"e63d09682c9a919a9ab347ff27d197f2a3c10a18",family_runtime_enabled:true,family_gateway_base:gateway}),{status:200,headers:{"Content-Type":"application/json"}});
    }
    if(url.href.startsWith(gateway)){
      ...
      if(method==="POST"&&url.pathname.endsWith("/operator/managed-people/invitations")){
        state.created=true;
        return new Response(JSON.stringify({ok:true,invitation_id:"00000000-0000-4000-8000-000000000777",state:"MESSAGE_PENDING",duplicate:false,outbound}),{status:201,headers:{"Content-Type":"application/json"}});
      }
  ...
  async function run(){
    ...
    for(let i=0;i<120&&panel?.hidden;i++)await new Promise(r=>setTimeout(r,25));
    ...
    const consent=document.getElementById("familyContactConsent");if(consent)consent.checked=true;
    ...
    for(let i=0;i<160;i++){
      const ready=mode==="direct"
        ? /Einladung wird über WhatsApp zugestellt/.test(status?.textContent||"")
        : !!status?.querySelector('a[href^="https://wa.me/"]');
      if(ready)break;
      await new Promise(r=>setTimeout(r,25));
    }
    let copyGreen=true,confirmGreen=true;
    if(mode==="activation"){
    ...
    const relevant=[...document.querySelectorAll("#familyOwnerPanel button,#familyOwnerPanel input,#familyOwnerPanel select,#familyPersonFormStatus a")].filter(el=>!el.closest("[hidden]"));
    const overflow=document.documentElement.scrollWidth>innerWidth+1||relevant.some(el=>{const r=el.getBoundingClientRect();return r.width>0&&(r.left<-1||r.right>innerWidth+1)});
    const directGreen=mode!=="direct"||/Einladung wird über WhatsApp zugestellt/.test(status?.textContent||"");
    const activationGreen=mode!=="activation"||(/Einladung über WhatsApp aktivieren/.test(status?.textContent||"")&&confirmGreen&&copyGreen);
...
<script src="${familyJs}?v=family-premium-browser-smoke"></script>
</body></html>`;
}

const server=http.createServer((req,res)=>{
  const path=(req.url||"/").split("?")[0];
  if(path==="/direct.html"||path==="/activation.html"){
    res.writeHead(200,{"Content-Type":"text/html; charset=utf-8"});
    res.end(page(path.includes("direct")?"direct":"activation"));return;
  }
  if(path===familyJs){
    res.writeHead(200,{"Content-Type":"application/javascript; charset=utf-8"});
    res.end(fs.readFileSync("assets/family-owner-sponsored-access.js","utf8"));return;
  }
  ...
  try{
    for(const width of [1440,390]){
      ...
      for(const mode of ["direct","activation"]){
        ...
        assert.match(r.stdout,new RegExp(`data-family-premium-browser-smoke="${mode}-green"`));
        assert.match(r.stdout,/data-family-premium-overflow="none"/);
        if(mode==="activation")assert.match(r.stdout,/data-family-premium-clipboard="green"/);
      }
    }
  } finally {
