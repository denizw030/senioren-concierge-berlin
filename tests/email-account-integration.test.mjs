...
test("status loading can never make Google Connect inert",()=>{
  assert.match(js,/function setBusy\(value, includeConnect = false\)/);
  assert.match(js,/if \(includeConnect && connectButton/);
  assert.match(js,/loading = true;\s*setBusy\(true\);/s);
  assert.match(js,/setBusy\(true, true\);\s*connectButton\.textContent = "Verbindung wird gestartet/s);
});

test("email gateway cannot hang forever during account status load",()=>{
  assert.match(js,/signal: AbortSignal\.timeout\(8000\)/);
});

test("connection errors keep Google Connect available when session exists",()=>{
  assert.match(js,/selectedProvider = "GOOGLE";\s*connectButton\.hidden = false;\s*connectButton\.disabled = !sessionToken\(\)/s);
  assert.match(konto,/email-account-integration\.js\?v=8/);
});
