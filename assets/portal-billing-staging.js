(()=>{
  ...
  function renderLoading(){
    if($('billingState')) $('billingState').innerHTML='<p>Tarif- und Nutzungsdaten werden geladen …</p>';
  }

  function renderError(){
    if($('billingState')) $('billingState').innerHTML='<div class="notice">Tarif- und Nutzungsdaten konnten gerade nicht geladen werden. Es wurde nichts verändert.</div>';
  }
  ...
  function renderState(state){
    ...
    const limits={
      ...
      Bilder:plan?.image_generation_limit,
      Dokumente:plan?.document_digitization_limit,
      Erinnerungen:plan?.reminder_limit,
      'Voice-Minuten':plan?.voice_minutes_limit,
      'Outbound-Voice-Minuten':plan?.outbound_voice_minutes_limit
    };
    ...
    $('billingState').innerHTML=`
      <div class="billing-grid">
        ...
        <div class="billing-card"><span>PAY PER USE</span><strong>${bool(pref.payg_enabled)}</strong><small>Nur nach ausdrücklicher Freigabe</small></div>
      ...
      <p class="tiny">Kostenpflichtige Zusatznutzung entsteht nur, wenn PAYG bzw. Overage ausdrücklich aktiviert ist. Ein fehlgeschlagener Zahlungsstatus darf keine unbegrenzte weitere kostenpflichtige Nutzung erzeugen.</p>`;
  }
