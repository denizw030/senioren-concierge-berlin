...
// SENIOREN_HERO_VISIBILITY_GUARD_V2
(() => {
  const isSeniorPage = /(?:^|\/)(?:senioren-concierge)(?:\.html)?\/?$/.test(location.pathname);
  if (!isSeniorPage) return;

  const IMAGE = "/assets/lifestyle/senior-man-phone.webp?v=5";

  const repair = () => {
    const hero = document.querySelector(".senior-image-hero");
    const layout = hero?.querySelector(".senior-hero-layout");
    const copy = layout?.querySelector(".senior-hero-copy");
    let photo = layout?.querySelector(".senior-hero-photo");
    if (!hero || !layout || !copy) return;

    if (!photo) {
      photo = document.createElement("figure");
      photo.className = "senior-hero-photo";
      layout.prepend(photo);
    }

    let img = photo.querySelector("img");
    if (!img) {
      img = document.createElement("img");
      img.alt = "Älterer Mensch nutzt zu Hause ein Smartphone mit persönlicher Unterstützung";
      photo.appendChild(img);
    }
    if (!/senior-man-phone\.webp/i.test(img.getAttribute("src") || "")) img.src = IMAGE;
    ...
    layout.style.setProperty("gap","clamp(28px,5vw,72px)","important");
    layout.style.setProperty("grid-template-columns", desktop ? "minmax(300px,.95fr) minmax(330px,1.05fr)" : "1fr","important");
    ...
    photo.style.setProperty("z-index","2","important");
    photo.style.setProperty("width","100%","important");
    photo.style.setProperty("max-width", desktop ? "620px" : "760px","important");
    photo.style.setProperty("min-width","0","important");
    photo.style.setProperty("min-height", desktop ? "clamp(430px,48vw,640px)" : "320px","important");
    photo.style.setProperty("aspect-ratio", desktop ? "4 / 5" : "4 / 3","important");
    photo.style.setProperty("margin","0","important");
    photo.style.setProperty("overflow","hidden","important");
    photo.style.setProperty("border-radius", desktop ? "34px" : "24px","important");
    photo.style.setProperty("background", '#efe5d2 url("' + IMAGE + '") center center / cover no-repeat', "important");
    photo.style.setProperty("box-shadow","0 30px 72px rgba(73,52,18,.16)","important");
    photo.style.setProperty("grid-column","1","important");
    photo.style.setProperty("grid-row","1","important");
    ...
    copy.style.setProperty("grid-column", desktop ? "2" : "1","important");
    copy.style.setProperty("grid-row", desktop ? "1" : "2","important");
    copy.style.setProperty("max-width","700px","important");
    ...
    if (img.dataset.nwHeroErrorBound !== "1") {
      img.dataset.nwHeroErrorBound = "1";
      img.addEventListener("error", () => {
        img.style.setProperty("display","none","important");
        photo.dataset.imageFallback = "background";
      });
    }

    hero.dataset.nwSeniorHeroRepaired = "1";
  };
  ...
  addEventListener("pageshow", repair);
})();
