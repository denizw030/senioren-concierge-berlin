(() => {
  ...
  async function boot() {
    ...
    // A URL alone is intentionally insufficient to enable sending. The browser
    // must not invent request/response semantics for the central Core. Enable
    // only after the canonical browser gateway contract is published together
    // with an explicit client contract version.
