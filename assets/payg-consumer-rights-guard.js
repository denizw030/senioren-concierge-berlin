(() => {
  ...
  const ALLOWED_WITHDRAWAL_ORIGINS = new Set(["https://nahwerkconcierge.com","https://www.nahwerkconcierge.com"]);
  ...
  function normalizeConsumerRights(raw) {
    ...
    const legalTextVersion = String(source.legal_text_version || "").trim();
    const legalTextSha256 = String(source.legal_text_sha256 || "").trim().toLowerCase();
    const ready = source.contract_version === CONTRACT_VERSION && source.authoritative === true && source.electronic_withdrawal_function === true && Boolean(withdrawalUrl) && source.immediate_performance_consent_evidence === true && source.order_confirmation_durable_medium === true && Boolean(legalTextVersion) && /^[0-9a-f]{64}$/.test(legalTextSha256);
    return Object.freeze({
      ...
      legal_text_sha256:legalTextSha256,
      ready
    });
  }
  ...
  function renderGate() {
    ...
    if (box) {
      ...
      box.append(strong,document.createTextNode(ready
        ? " Elektronischer Widerruf, versionierter Sofortausführungs-Nachweis und dauerhafte Bestellbestätigung sind autoritativ freigegeben."
        : " Kostenpflichtige PAYG-Aufträge bleiben gesperrt, bis der autoritative PROD-Vertrag elektronischen Widerruf, Sofortausführungs-Nachweis, versionierten Rechtstext und dauerhafte Bestellbestätigung bestätigt."));
  ...
  function ensureDialog() {
    ...
    dialog.innerHTML = `<form method="dialog" style="max-width:620px;padding:4px;display:grid;gap:16px"><div><div class="eyebrow">PAYG · Verbraucherrecht</div><h2 id="paygImmediatePerformanceTitle" style="margin:6px 0 8px">Sofortige Ausführung bestätigen</h2><p id="paygImmediatePerformanceOrder" style="margin:0;color:#555"></p></div><label style="display:flex;gap:10px;align-items:flex-start"><input id="paygImmediateStart" type="checkbox" /> <span>Ich verlange ausdrücklich, dass NAHWERK vor Ablauf der Widerrufsfrist mit dieser kostenpflichtigen Dienstleistung beginnt.</span></label><label style="display:flex;gap:10px;align-items:flex-start"><input id="paygWithdrawalExpiryAck" type="checkbox" /> <span>Mir ist bekannt, dass mein Widerrufsrecht bei vollständiger Vertragserfüllung erlischt, wenn die gesetzlichen Voraussetzungen erfüllt sind.</span></label><div style="display:flex;gap:10px;flex-wrap:wrap"><button class="btn red" id="paygConsumerRightsConfirm" type="button">Erklärungen bestätigen</button><button class="btn light" value="cancel" type="submit">Abbrechen</button></div></form>`;
    ...
    dialog.querySelector("#paygConsumerRightsConfirm")?.addEventListener("click",() => {
      ...
      pendingEvidence = {
        ...
        legal_text_version:contract.legal_text_version,
        legal_text_sha256:contract.legal_text_sha256,
  ...
  function openConsentDialog(button) {
    ...
    if (summary) summary.textContent = `Diese Erklärungen gelten nur für die ausgewählte Preisfreigabe ${button.textContent?.trim() || ""}. Rechtstext-Version: ${contract?.legal_text_version || ""}.`;
    current.showModal();
  }

  function requestMatchesPayg(input) {
    try { return new URL(input instanceof Request ? input.url : String(input),location.href).href === PAYG_URL; }
    catch { return false; }
  }
  ...
  window.fetch = async (input,init = {}) => {
    ...
    if (isPayg && method === "POST" && typeof init?.body === "string") {
      ...
      if (body?.action === "approve_quote") {
        ...
        if (!(contract?.ready === true && pendingEvidence?.quote_id === quoteId)) {
          return new Response(JSON.stringify({ ok:false,status:"consumer_rights_evidence_required" }),{ status:409,headers:{ "Content-Type":"application/json; charset=utf-8","Cache-Control":"no-store" } });
        }
    ...
    if (isPayg && method === "GET") {
      try { contract = normalizeConsumerRights(await response.clone().json()); }
      catch { contract = null; }
      renderGate();
    }
    return response;
  ...
  window.NAHWERKPaygConsumerRightsGate = Object.freeze({ contractVersion:CONTRACT_VERSION,normalizeConsumerRights,isReady:() => contract?.ready === true });
